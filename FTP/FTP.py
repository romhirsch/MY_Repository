...
def servClient(client, address):
      #file_name = [f for f in os.listdir() if f!="FTP.py" and f!="run.bat"][0] #from list files in dir - look for the file to transfer
      files = [f for f in os.listdir() if f!="FTP.py" and f!="run.bat"]
      client.send(str(len(files)).encode())
...
def runClient(soc):
      numOfFiles = int(soc.recv(1024).decode())
      print(f'numOfFiles: {numOfFiles}')
...
def Main():
      ...
      server = True
      port = 2104 #get port val from bat file
      ip = "0.0.0.0" # set ip address according to role
